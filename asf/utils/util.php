...
| (?<=[\xF0-\xF7][\x80-\xBF])[\x80-\xBF](?![\x80-\xBF]) # Short 4 byte sequence (2)
    ...
    //my additions...
    public static function var_dump_die($var) {
        self::var_dump($var);
        die();
    }
    ...
    public static function last_index_of($haystack, $needle, $offset = 0) {
     ...
     * 
     * @param string $val
     * @return non zero value on true
     */
    public static function is_str_empty($val) {
        return (trim($val) == '');
    }
    ...
    public static function end_slash($url) {
    ...
    public static function is_array_of_ints($array) {
    ...
    public static function sanitize_recursive($s) {
        if (is_array($s)) {
            return array_map(array('self', 'sanitize_recursive'), $s);
        } else {
            return htmlspecialchars($s);
        }
    }
    
    public static function gen_uuid($sep='-') {
    ...
    public static function unset_by_value(&$array, $val = '') {
    ...
    public static function mysqlDateTime($timestamp=NULL) {
        return self::dateTimeInGMT($timestamp);
    }

    public static function toDate($mysqlDateTime, $format="M j Y") {
        return self::gmtDatetimeToLocal($mysqlDateTime, $format);
    }

    public static function toISO_8601Date($mysqlDateTime) {
        return self::gmtDatetimeToLocal($mysqlDateTime, "Y-m-d");
    }

    public static function dateTimeInGMT($ts=NULL) {
        if (is_null($ts)) {
            $ts = time();
        }
    ...
    public static function gmtDatetimeToLocal($datetime, $format='Y-m-d H:i:s') {
